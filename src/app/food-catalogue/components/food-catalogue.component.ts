...
export class FoodCatalogueComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe(params => {
      ...
      if (idParam !== null) {
        this.restaurantId = +idParam; // safely cast to number
        this.getFoodItemsByRestaurant(this.restaurantId);
      } else {
        // Handle the case where the id is not found in the URL
        console.error("Restaurant ID is missing!");
        // Optionally, you could redirect or show an error message here.
      }
    });
...
/** 
  onCheckOut() {
    ...
    this.orderSummary = {
      ...
      onCheckOut() {
        // Ensure foodItemResponse and foodItemResponse.restaurant are valid before proceeding
        if (this.foodItemResponse && this.foodItemResponse.restaurant) {
          this.orderSummary = {
            footItemList: this.foodItemCart,
            restaurant: this.foodItemResponse.restaurant
          };
          this.router.navigate(['/orderSummary'], { queryParams: { data: JSON.stringify(this.orderSummary) } });
        } else {
          console.error("Food item response or restaurant data is missing.");
          // Optionally, show an error message to the user or redirect to another page
        }
  
    this.orderSummary.footItemList = this.foodItemCart;
